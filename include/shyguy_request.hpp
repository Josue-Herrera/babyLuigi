...
namespace cosmos::inline v1
{
    ...
    inline std::string_view to_string_view(request_type const type)
    {
        ...
        switch (type)
        {
            case request_type::dag:
                return "dag"sv;
            case request_type::task:
                return "task"sv;
            case request_type::unset:
                return "unset"sv;
    ...
    inline request_type to_request_type(std::string const &str)
    {
        ...
        if (str == "dag"s)
            return request_type::dag;
        if (str == "task"s)
            return request_type::task;
        if (str == "unset"s)
            return request_type::unset;
    ...
    inline std::string_view to_string_view(task_type const type)
    {
        ...
        switch (type)
        {
            ...
            case task_type::shell:
                return "shell"sv;
            case task_type::unset:
                return "unset"sv;
            case task_type::unknown:
                return "unknown"sv;
            default:
                return "not-set"sv;
        }
    ...
    inline task_type to_task_type(std::string const &str)
    {
        using namespace std::string_literals;
        if (str == "python"s)
            return task_type::python;
        if (str == "shell"s)
            return task_type::shell;
        if (str == "unset"s)
            return task_type::unset;
...
namespace cosmos::inline v1
{
    ...
    inline command_enum to_command_enum(std::string const &str)
    {
        using namespace std::string_literals;
        if (str == "create"s)
        ...
        if (str == "error"s)
            return command_enum::error;
        return command_enum::not_set;
    }
    ...
    inline auto has_schedule(shyguy_dag const &request) -> bool { return exists(request.schedule); }
...
namespace nlohmann
{
    ...
    struct adl_serializer<cosmos::shyguy_request>
    {
        ...
        {
            request.command = cosmos::to_command_enum(j.value("command", "not-set"));
            ...
            else if (j.contains("type") and j.contains("value"))
            {
                switch (auto const type = j.at("type").get<std::string>(); cosmos::to_request_type(type))
                {
                    case cosmos::request_type::dag:
                        request.data = j.at("value").get<cosmos::shyguy_dag>();
                        break;
                    case cosmos::request_type::task:
                        request.data = j.at("value").get<cosmos::shyguy_task>();
                        break;
